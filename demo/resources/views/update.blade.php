...
<body>
    ...
    <h1 class="text-success m-3 ">
       ...
       <secton class="text-center m-3">
        <form  class="w-75 m-auto text-center border border-dark p-2" method="POST" action="{{ route('students.edit',$student->id) }}" >
            @csrf
            @method('put')
                            {{-- student name --}}
            ...
            <div class="mb-3">
                ...
                <input name='age' type="number" class="form-control" id="exampleInputAge"  value="{{ $student->age }}">
            </div>
                    {{-- student Address --}}
